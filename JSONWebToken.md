...
- JSON データをそのまま文字列としてクエリパラメータやリクエスト/レスポンスヘッダーに付与しないのか?

    - セキュリティ面で問題あり

    - JSON データをそのまま文字列として付与しようとすると、ヘッダーや URL が長くなりすぎる

        - URL や HTTP ヘッダーには文字数制限があるため長いヘッダーや URL ではリクエスト/レスポンスが送れなくなる

    → これらの問題を JWT および JWS で解決する

    - JWS: JSON Web Signature の略。JWT が本物か、もしくは中身が改ざんされていないかを検証する仕組みのこと
...
[JWTとJWSとは何か](https://qiita.com/soicchi/items/b8c87a3f2f384b3d06ff)
...
- JWT は以下の3つから構成される
    ...
    2. Payload 部 (要素)
        ...
        - Payload 部のデータ項目 (JSON のキーと値のペア) はクレームと呼ばれる
    ...
    3. Signature 部 (要素)
        ...
        - (BASE64URL エンコードされた) Header 部と Payload 部をピリオドで繋げた文字列を JWT の発行者のみが知るシークレット鍵を使ってハッシュ化した文字列をさらに BASE64URL エンコードしたもの
...
2. バックエンド側で送られてきた JWT の検証を行う
    ...
    - 復号化された Siganture 部 (= 「BASE64URL でエンコードされた Header 部」 + 「.」 + 「BASE64URL でエンコードされた Payload 部」) が送られてきた JWT の Header 部と Payload 部と一致するか検証
...
- BASE64 エンコードをかけた後に、URL で特別な意味を持つ記号「+」と「/」を他の記号に変換する方式 を BASE64URL エンコードと呼ぶ
...
- URL/URI のファイル名やクエリ文字列などの一部としては使用できない記号や文字を、使用できる文字の特殊な組み合わせによって表記すること
