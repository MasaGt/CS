...
もっと柔軟にIPアドレスのネットワーク部を決めれるようにするのがサブネット(subnetting)、その中で現在主流なものが[CIDR](#cidr)
...
ポイント
- クラスAのみネットワーク数は2^7 - 2
- クラスB,Cのネットワーク数は-2しなくていい(IBMのサイトのリンクを貼る) **と思ったが、いくつかのサイトではクラス
B,Cも-2するとの情報あり**  
    ->一応両方記述するが、重要なのは、ホスト数らしいので、そこも記述すること 
- ネットワークアドレス(ホスト部全て0)と、ブロードキャストアドレス(ホスト部全て1)は別項目の1つとして設ける
- サブネットマスク
- CIDR
