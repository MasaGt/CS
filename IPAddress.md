...
具体的には、複数のネットワークで共通するネットワーク部分をまとめ、新しいネットワーク部(スーパーネット部分)としてくくり、それを元に新たにIPアドレス (スーパーネットのIPアドレス) を作ってそこにまとめる
...
$\color{yellow}{狭義の意味でのCIDR}$

**スーパーネッティングの1種としてのCIDR**

重要な前提情報
- $\color{red}{CIDRが考案された時はまだクラスフルなIPアドレスが主流}$
...
クラスAのIPアドレスほど多くのホストを接続するわけではない
=>クラスAのIPはいらない

クラスCのIPで接続できるホストよりも多く接続したい
=>クラスCのIPはいらない

[結果]
多くの会社でクラスBに属するIPアドレスの取得をするようになり、クラスBのIPアドレスが枯渇する
```

上記の問題を解決するためにCIDRが登場

```
クラスCのネットワークを複数束ねて1つのネットワークにすれば1つの大きなネットワークに集約できる
的な考え方
```
...
```
例:
1つの会社で以下の複数のクラスCに属するIPアドレス(ネットワーク)を使用しているとする
200.0.1.0
200.0.2.0
200.0.3.0
200.0.4.0
...
[各IPアドレスのネットワーク部で共通部分をネットワーク部にした新たなIPアドレスを創出]

各のIPアドレスは23ビットまで
11001000 00000000 00000
が共通している
↓
11001000 00000000 00000 をネットワークーク部にしたIPアドレス

200.0.0.0をスーパーネットのIPアドレスとすれば以下の画像のように1つのネットワークにまとめることができる
```
...
しかし、元のクラスCのネットワークたちのネットワーク部は先頭24ビットまでだったのに対し、新たらしいネットワークの IP アドレスは先頭23ビットまでがネットワーク部

$\color{red}{IPアドレスを一目見てどこまでがネットワーク部なのかがわからない}$
...
上記の考え/仕組みで作成されるIPアドレスを**クラスレスアドレス**と呼ぶ
...
例: 192.168.0.0/16 でIPアドレスを構築した会社があるとする
